import React, { Component } from "react";

import * as lib from "../../utils/canvasUtils";

import "./Wrapper.scss";

export default function Wrapper(WrapperedComponent) {
  return class extends Component {
    constructor(props) {
      super(props);

      this.state = {
        t: 0,
        time: 0,
        points: [],
        color: props.color || "#A39B84",
        stroke: props.stroke || "rgba(200,69,200,.6)"
      };
      this.canvasRef = React.createRef();
      this.updateAnimationState = this.updateAnimationState.bind(this);
    }

    componentDidMount() {
      const ctx = this.canvasRef.current.getContext("2d");
      const size = lib.dim(this.canvasRef.current);
      const genereadedPoints = lib.generate(
        20,
        lib.dotInDiv,
        size,
        2,
        2,
        this.state.color
      );
      ctx.fillStyle = this.color;
      ctx.strokeStyle = this.state.stroke;
      this.setState({ size: size, points: genereadedPoints, ctx });
      this.rAF = requestAnimationFrame(this.updateAnimationState);
    }

    updateAnimationState(timestamp) {
      const { ctx, points, size, time } = this.state;
      lib.clearCanvas(size, ctx);
      lib.findClosest(points);
      points.forEach(point => {
        lib.drawCircle(ctx, point);
        lib.drawLine(ctx, point, points[point.closest[1].index]);
        lib.drawLine(ctx, point, points[point.closest[2].index]);
        lib.drawLine(ctx, point, points[point.closest[3].index]);
        lib.update2(point, time);
      });
      this.setState(prevState => ({
        t: prevState.t + 1000 / 60,
        time: prevState.time + timestamp
      }));
      this.rAF = requestAnimationFrame(this.updateAnimationState);
    }

    componentWillUnmount() {
      cancelAnimationFrame(this.rAF);
    }

    render() {
      return (
        <div className="wrapper">
          <WrapperedComponent {...this.props} />
          <canvas
            className="wrapper__canvas"
            width={700}
            height={170}
            ref={this.canvasRef}
          />
        </div>
      );
    }
  };
}

// function animation(points, ctx, size, t = 0, time = 0) {
//   return (function loop(timestamp) {
//     clearCanvas(size, ctx);
//     points.forEach(point => {
//       drawCircle(ctx, point);
//       drawLine(ctx, point, points[point.closest[1].index]);
//       drawLine(ctx, point, points[point.closest[2].index]);
//       drawLine(ctx, point, points[point.closest[3].index]);
//       update3(point, time);
//     });

//     findClosest(points);

//     t += 1000 / 60;
//     time = +timestamp;
//     requestAnimationFrame(loop);
//   })();
// }

// function update(point, time) {
//   let x = point.destination.clientX - point.x;
//   let y = point.destination.clientY - point.y;
//   let m = y / x;
//   let b = point.y - m * point.x;

//   point.x =
//     point.destination.clientX > point.x
//       ? point.x + point.speed * point.acceleration
//       : point.x - point.speed * point.acceleration;
//   point.y = m * point.x + b;
//   return point;
// }

// function update2(point, time) {
//   point.x =
//     point.destination.x > point.x
//       ? point.x + point.speed * point.acceleration
//       : point.x - point.speed * point.acceleration;
//   point.y =
//     point.destination.y > point.y
//       ? point.y + point.speed * point.acceleration
//       : point.y - point.speed * point.acceleration;
//   return point;
// }
// function update3(point, time) {
//   point.x = Number.parseInt(point.x + point.speed * point.acceleration);
//   point.y = Number.parseInt(
//     point.destination.y > point.y
//       ? point.y + point.speed * point.acceleration
//       : point.y - point.speed * point.acceleration
//   );
//   return point;
// }

// function generate(n, fn, div, speed, radius, color) {
//   const result = [];
//   while (n--) {
//     result.push(fn(div, speed, radius, color));
//   }
//   return result;
// }

// function dotInDiv(dimentions, speed, radius, color) {
//   const point = randomPoint(dimentions);
//   Object.assign(point, { markDestination });

//   return {
//     ...point,
//     speed: randomOf(speed, 1),
//     radius: randomOf(radius, 1),
//     acceleration: 0.5,
//     destination: randomPoint(dimentions),
//     color
//   };
// }

// function randomPoint(dimentions) {
//   return {
//     x: Math.floor(randomOf(dimentions.width)),
//     y: Math.floor(randomOf(dimentions.height))
//   };
// }
// const dim = ({ clientHeight, clientWidth }) => ({
//   width: clientWidth,
//   height: clientHeight
// });

// function randomOf(value, start = 0) {
//   return Math.floor(Math.random() * (value - start + 1)) + start;
// }

// function markDestination(destination) {
//   Object.assign(this, { destination });
// }

// function findClosest(points) {
//   const distances = points.map(point => {
//     let distanceValues = [];
//     points.forEach(pointB => distanceValues.push(distance(point, pointB)));
//     return distanceValues;
//   });

//   const closest = distances.map(distanceArr =>
//     distanceArr
//       .map((distance, index) => {
//         return {
//           distance,
//           index
//         };
//       })
//       .sort((a, b) => a.distance - b.distance)
//   );

//   return points.reduce((aggregator, point, index) => {
//     const updatedPoint = Object.assign(point, {
//       distances: distances[index],
//       closest: closest[index]
//     });
//     aggregator.push(updatedPoint);
//     return aggregator;
//   }, []);
// }

// const distance = (a, b) =>
//   Math.sqrt(Math.pow(a.x - b.x, 2) + Math.pow(a.y - b.y, 2)).toFixed(2);

// const clearCanvas = ({ width, height }, ctx) =>
//   ctx.clearRect(0, 0, width, height);

// function drawCircle(ctx, center, radius = 3, color = "rgba(100,100,100,.8)") {
//   let defaultColor = ctx.fillStyle;
//   ctx.fillStyle = color || ctx.fillStyle;
//   ctx.beginPath();
//   ctx.arc(center.x, center.y, radius, 0, 2 * Math.PI);
//   ctx.closePath();
//   ctx.fill();
//   ctx.fillStyle = defaultColor;
// }

// function drawLine(ctx, a, b) {
//   ctx.beginPath();
//   ctx.moveTo(a.x, a.y);
//   ctx.lineTo(b.x, b.y);
//   ctx.stroke();
// }
